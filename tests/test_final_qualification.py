...
def test_full_qualification_acceptance_blocks_only_on_blocking_open_or_fail():
    root = Path(__file__).resolve().parents[1]
    workflow = (root / ".github" / "workflows" / "full-qualification.yml").read_text(encoding="utf-8")
    assert 'if ([int]$r.summary.blocking_open -gt 0 -or [int]$r.summary.blocking_fail -gt 0) {' in workflow
