    ...
    try:
        ...
        for statement in statements:
            if not statement.startswith('CREATE TRIGGER '):
                c.execute(statement)
        source_cols=[('source_kind',"TEXT DEFAULT 'website'"),('acquisition',"TEXT DEFAULT 'http'"),('verification_state',"TEXT DEFAULT 'unverified'"),('access_scope',"TEXT DEFAULT 'public'"),('terms_status',"TEXT DEFAULT 'not_reviewed'"),('failure_count',"INTEGER DEFAULT 0"),('last_error',"TEXT"),('next_retry_at',"TEXT"),('country',"TEXT"),('region',"TEXT"),('language',"TEXT"),('source_family',"TEXT"),('execution_capability',"TEXT DEFAULT 'manual'")]
        ...
        for col,ddl in [('source_lane',"TEXT DEFAULT 'REVIEW'"),('blacklist_reason','TEXT'),('blacklisted_at','TEXT'),('project_scan_interval_minutes','INTEGER DEFAULT 60'),('intelligence_scan_interval_minutes','INTEGER DEFAULT 720')]: _ensure_column(c,'sources',col,ddl)
        ...
        for col,ddl in [('country','TEXT'),('region','TEXT'),('language','TEXT'),('source_family','TEXT'),('execution_capability',"TEXT DEFAULT 'manual'"),('iran_policy_basis','TEXT'),('iran_policy_url','TEXT'),('policy_checked_at','TEXT'),('source_origin','TEXT'),('upstream_sources','TEXT')]: _ensure_column(c,'source_contracts',col,ddl)
        ...
        for col,ddl in [('source_lane',"TEXT DEFAULT 'REVIEW'"),('blacklist_reason','TEXT'),('blacklisted_at','TEXT'),('project_scan_interval_minutes','INTEGER DEFAULT 60'),('intelligence_scan_interval_minutes','INTEGER DEFAULT 720')]: _ensure_column(c,'source_contracts',col,ddl)
    ...
    try:
        ...
        for s in records:
            ...
            values=(s['name'],s['base_url'],s.get('adapter','json'),s.get('type','marketplace'),s.get('status','candidate'),
                    ...
                    s.get('execution_capability','manual'),original_policy_lane,int(bool(s.get('daily_scan'))),int(bool(s.get('needs_analysis'))),s.get('source_role'),
            ...
            c.execute("""INSERT INTO sources(name,base_url,adapter,type,status,iran_status,kyc_status,payment_status,execution_mode,proposal_limit,source_kind,acquisition,access_scope,terms_status,country,region,language,source_family,execution_capability,policy_lane,daily_scan,needs_analysis,source_role,iran_policy_basis,iran_policy_url,policy_checked_at,source_origin,upstream_sources,source_verification_state,iran_eligibility,kyc_requirement,payment_capabilities,payout_evidence_url,kyc_evidence_url,terms_evidence_url,last_verified_at,evidence_confidence,market_intelligence_value,execution_ready,discovery_basis)
                         ...
                         execution_mode=COALESCE(excluded.execution_mode,sources.execution_mode), proposal_limit=COALESCE(excluded.proposal_limit,sources.proposal_limit),country=excluded.country,region=excluded.region,language=excluded.language,source_family=excluded.source_family,execution_capability=excluded.execution_capability,
            ...
            c.execute("""INSERT INTO source_contracts(source,source_kind,acquisition,adapter,access_scope,verification_state,terms_status,verification_basis,updated_at,runtime_verification_state,country,region,language,source_family,execution_capability,policy_lane,daily_scan,needs_analysis,source_role,iran_policy_basis,iran_policy_url,policy_checked_at,source_origin,upstream_sources,source_verification_state,iran_eligibility,kyc_requirement,payment_capabilities,payout_evidence_url,kyc_evidence_url,terms_evidence_url,last_verified_at,evidence_confidence,market_intelligence_value,execution_ready,discovery_basis)
                         VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
                         ON CONFLICT(source) DO UPDATE SET source_kind=excluded.source_kind, acquisition=excluded.acquisition, adapter=excluded.adapter, access_scope=excluded.access_scope, verification_state=excluded.verification_state, terms_status=excluded.terms_status, verification_basis=excluded.verification_basis, updated_at=excluded.updated_at,country=excluded.country,region=excluded.region,language=excluded.language,source_family=excluded.source_family,execution_capability=excluded.execution_capability,policy_lane=excluded.policy_lane,daily_scan=excluded.daily_scan,needs_analysis=excluded.needs_analysis,source_role=excluded.source_role,iran_policy_basis=excluded.iran_policy_basis,iran_policy_url=excluded.iran_policy_url,policy_checked_at=excluded.policy_checked_at,source_origin=excluded.source_origin,upstream_sources=excluded.upstream_sources,source_verification_state=excluded.source_verification_state,iran_eligibility=excluded.iran_eligibility,kyc_requirement=excluded.kyc_requirement,payment_capabilities=excluded.payment_capabilities,payout_evidence_url=excluded.payout_evidence_url,kyc_evidence_url=excluded.kyc_evidence_url,terms_evidence_url=excluded.terms_evidence_url,last_verified_at=excluded.last_verified_at,evidence_confidence=excluded.evidence_confidence,market_intelligence_value=excluded.market_intelligence_value,execution_ready=excluded.execution_ready,discovery_basis=excluded.discovery_basis""",
                      (s['name'],s.get('source_kind','website'),s.get('acquisition','http'),s.get('adapter','json'),s.get('access_scope','public'),s.get('verification_state','unverified'),s.get('terms_status','not_reviewed'),s.get('verification_basis'),now,None,s.get('country'),s.get('region'),s.get('language'),s.get('source_family',s.get('acquisition','http')),s.get('execution_capability','manual'),original_policy_lane,int(bool(s.get('daily_scan'))),int(bool(s.get('needs_analysis'))),s.get('source_role'),s.get('iran_policy_basis'),s.get('iran_policy_url'),s.get('policy_checked_at'),s.get('source_origin'),json.dumps(s.get('upstream_sources',[]),ensure_ascii=False),s.get('source_verification_state','DISCOVERED'),s.get('iran_eligibility',s.get('iran_status','UNKNOWN')),s.get('kyc_requirement','UNKNOWN'),json.dumps(s.get('payment_capabilities',[]),ensure_ascii=False),s.get('payout_evidence_url'),s.get('kyc_evidence_url'),s.get('terms_evidence_url'),s.get('last_verified_at'),float(s.get('evidence_confidence',0) or 0),s.get('market_intelligence_value','MEDIUM'),int(bool(s.get('execution_ready'))),s.get('discovery_basis')))
