...
from .source_parsers import parse_arbeitnow, parse_jobicy, parse_jobremotely, parse_remoteok, parse_rss, parse_hh, parse_html_jobs, parse_telegram_bot
PARSERS={
 ...
 'arbeitnow_json':lambda body,url:parse_arbeitnow(body,url),
...
def content_type_ok(adapter,content_type):
    ...
    if adapter in {'json','remoteok_json','jobicy_json','jobremotely_json','arbeitnow_json','hh_json','telegram_bot_json'}: return c in {'application/json','application/problem+json','text/json','text/plain',''} or c.endswith('+json')
